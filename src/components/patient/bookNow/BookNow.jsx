...
const BookNow = ({ date, id, email }) => {
    ...
    const getDoctorsDetails = async () => {
        try {
            const dataset = await axios.get(`https://doctorsapp-backend-3.onrender.com/getdoctoravailability`, { params: { id: id } });
    ...
    const getAlreadyBookedData = async () => {
        try {
            const dataset = await axios.get(`https://doctorsapp-backend-3.onrender.com/getalreadybookedslots`, { params: { id: id } });
            console.log(dataset);
    ...
    const bookSlot = async () => {
        ...
        try {
            ...
            await axios.post(`https://doctorsapp-backend-3.onrender.com/submitbookings`, bookingData, { headers: { 'Authorization': `${token}` } });
            alert('slot booked');
        } catch (error) {
            alert(error);
        }
