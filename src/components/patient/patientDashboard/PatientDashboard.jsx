...
const PatientDashboard = () => {
  ...
  const getdoctors = async () => {
    try {
      const dataset = await axios.get("https://doctorsapp-backend-3.onrender.com/getapproveddoctors");
        ...
        doctorDetails.map((item, index) => (
          <div key={index}>
            <div className='itemclass'>
              <img className='patientdashboardimage' src={`https://doctorsapp-backend-3.onrender.com/` + item.profile_image} alt={item.name}></img>
