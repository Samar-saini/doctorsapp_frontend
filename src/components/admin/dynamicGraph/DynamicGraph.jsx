...
const DynamicGraph = () => {
    ...
    const doctorAndPatientCount = async () => {
        ...
        try {
            const response = await axios.get(`https://doctorsapp-backend-3.onrender.com/getTotalPatientByDoctor`, { headers: { 'Authorization': `${token}` } });
    ...
    const dateAndPatientCount = async () => {
        const token = localStorage.getItem('authToken');
        try {
            const response = await axios.get(`https://doctorsapp-backend-3.onrender.com/getTotalPAtientByDate`, { headers: { 'Authorization': `${token}` } });
