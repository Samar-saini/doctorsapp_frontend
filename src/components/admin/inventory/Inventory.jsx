...
const Inventory = () => {
  ...
  const getInventoryData = async () => {
    try {
      ...
      const response = await axios.get(`https://doctorsapp-backend-3.onrender.com/getinventorydata`, {
        headers: {
          'Authorization': `${token}`
        }
      });
  ...
  const handleAdd = async(index) => {
    ...
    const token = localStorage.getItem('authToken');
    await axios.patch(`https://doctorsapp-backend-3.onrender.com/updateinventory`, updatedInventory[index], {
      headers: {
        'Authorization': `${token}`
      }
    });
  ...
  const handleImport = async(e) => {
    ...
    reader.onload = async (event) => {
      ...
      const token = localStorage.getItem('authToken');
      await axios.post(`https://doctorsapp-backend-3.onrender.com/bulkinventory`, {data:jsonData}, {
        headers: {
          'Authorization': `${token}`
        }
      });
